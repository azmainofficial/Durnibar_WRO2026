...
def api_start_challenge():
    ...
    if mode in ["OPEN_CHALLENGE", "OBSTACLE_CHALLENGE", "PARKING", "IDLE", "PARKING_OUT", "BLOCK_RUN", "DYNAMIC_PARKING_OUT", "INTEGRATED_ROUND2"]:
        challenge_mode = mode
        ...
        try:
            ...
            while True:
                ...
                if challenge_mode != prev_challenge_mode:
                    ...
                    if challenge_mode in ["OPEN_CHALLENGE", "OBSTACLE_CHALLENGE", "PARKING_OUT", "DYNAMIC_PARKING_OUT", "BLOCK_RUN", "INTEGRATED_ROUND2"]:
                        lap_count = 0
                        waypoint_index = 0
                        cumulative_yaw = 0.0
                        start_dist = esp_dist
                ...
                # STATE 3.8: DYNAMIC BLOCK SEQUENCE RUNNER (Block programming style & Integrated Round 2)
                elif challenge_mode in ["BLOCK_RUN", "INTEGRATED_ROUND2"]:
                    now = time.time()
                    if current_block_index >= len(active_block_sequence):
                        ser.write(b"S\n")
                        if challenge_mode == "INTEGRATED_ROUND2":
                            challenge_mode = "OBSTACLE_CHALLENGE"
                            # Initialize obstacle run parameters!
                            lap_count = 0
                            waypoint_index = 0
                            cumulative_yaw = 0.0
                            start_dist = esp_dist
                            ...
                            turn_direction = 0
                            turn_start_yaw = 0.0
                            # Reset odom and start main obstacle run
                            ser.write(b"R\nY 0\nG 1\nL 0\nB\n")
                            print("[INTEGRATED SUCCESS] Parking exit block sequence complete! Transitioning to Obstacle Challenge.")
                        else:
                            challenge_mode = "IDLE"
