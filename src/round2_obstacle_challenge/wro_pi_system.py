...
# ===================== GLOBAL WATCHDOGS =====================
last_lidar_scan_time = time.time()
...
def camera_loop():
    ...
    def _open_camera():
        ...
        try:
            ...
            if c.isOpened():
                # Use hardware-compressed MJPG format to reduce USB bandwidth by 95%
                ...
                if ret and frame is not None:
                    print(f"[CAMERA SUCCESS] Fifine K420 online on /dev/video{target_id} (640x360 MJPG hardware compressed)")
                    return c
                c.release()
        except Exception as e:
    ...
    while True:
        ...
        if not ret or raw_frame is None:
            ...
            if consecutive_drops >= 5:
                ...
                cap = None
                consecutive_drops = 0
                time.sleep(3.0)  # Give USB bus 3s to fully settle before reopening camera
            else:
                time.sleep(0.05)
            continue
        ...
        if current_mask_mode == 'green':
            ...
            active_mask = cv2.inRange(
                hsv,
                np.array([cg.get('h_min', 35), cg.get('s_min', 80), cg.get('v_min', 50)], np.uint8),
                np.array([cg.get('h_max', 85), cg.get('s_max', 255), cg.get('v_max', 255)], np.uint8)
            )
        elif current_mask_mode == 'pink':
            cp = cfg.get('pink', {})
            active_mask = cv2.inRange(
                hsv,
                np.array([cp.get('h_min', 140), cp.get('s_min', 50), cp.get('v_min', 80)], np.uint8),
                np.array([cp.get('h_max', 170), cp.get('s_max', 255), cp.get('v_max', 255)], np.uint8)
            )
        else:
            cr1 = cfg.get('red1', {})
            cr2 = cfg.get('red2', {})
            m1 = cv2.inRange(
                hsv,
                np.array([cr1.get('h_min', 0), cr1.get('s_min', 150), cr1.get('v_min', 100)], np.uint8),
                np.array([cr1.get('h_max', 10), cr1.get('s_max', 255), cr1.get('v_max', 255)], np.uint8)
            )
            m2 = cv2.inRange(
                hsv,
                np.array([cr2.get('h_min', 170), cr2.get('s_min', 150), cr2.get('v_min', 100)], np.uint8),
                np.array([cr2.get('h_max', 180), cr2.get('s_max', 255), cr2.get('v_max', 255)], np.uint8)
            )
        ...
        for det in detections:
            ...
            # LiDAR tower angular lookup (cx < cx_center is LEFT / positive angle, cx > cx_center is RIGHT / negative angle)
            angle_off    = ((cx_center - det['cx']) / cx_center) * half_fov
            lidar_dist_cm = None
...
# ===================== RPLIDAR C1 THREAD =====================
def lidar_loop():
    global latest_lidar_points, latest_telemetry, last_lidar_scan_time
    ...
    while True:
        try:
            if not os.path.exists(port):
                print(f"[LIDAR] Waiting for device at {port}...")
                time.sleep(2)
                continue

            # Open serial port - RPLiDAR C1 motor is controlled by DTR pin
            # DTR=False (voltage HIGH on hardware) = motor ON, DTR=True = motor OFF
            ser = serial.Serial(port, baud, timeout=1.5,
                                dsrdtr=False, rtscts=False)
            ser.dtr = False   # MOTOR ON - must be False to spin the LiDAR
            ser.rts = False   # Keep RTS low too
            time.sleep(1.0)   # Give motor 1 full second to reach scan speed
            ...
            # GET_INFO handshake to verify device is alive
            ser.write(b'\xa5\x50')
            time.sleep(0.25)
            resp = ser.read(27)

            if len(resp) < 7 or resp[0:2] != b'\xa5\x5a':
                print(f"[LIDAR WARN] RPLiDAR GET_INFO bad response (len={len(resp)} header={resp[:4].hex() if resp else 'empty'}), retrying in 2s...")
                try: ser.close()
                except: pass
                time.sleep(2.0)
                continue
            ...
            while True:
                ...
                if len(points) >= pts_threshold:
                    # Wall distances & front diagonal corner sectors
                    ...
                    fl_d    = [d for a, d in points if  25 <= a <=  55]
                    fr_d    = [d for a, d in points if 305 <= a <= 335]
                    ...
                    avg_fl    = round((sum(fl_d)    / len(fl_d))    / 1000.0, 2) if fl_d    else 0.0
                    avg_fr    = round((sum(fr_d)    / len(fr_d))    / 1000.0, 2) if fr_d    else 0.0
                    ...
                    with state_lock:
                        ...
                        latest_telemetry['lidar_fl_m']       = avg_fl
                        latest_telemetry['lidar_fr_m']       = avg_fr
...
def esp32_loop():
    ...
    stuck_reason = ""
    last_recovery_cooldown = 0.0  # BUGFIX: init here so 'in locals()' check is reliable
    ...
    while True:
        try:
            ...
            while True:
                ...
                # BUTTON 1: RUN THE BOT (Starts Obstacle Challenge 3 Laps)
                if btn1_pressed:
                    ...
                    stuck_recovery_mode = False
                    ser.write(b"Y 0\nG 1\nL 0\nB\n")
                    print("[HARDWARE BUTTON 1] Pressed -> RUNNING BOT (Obstacle Challenge, 3 Laps, Green/Red Evasion, Speed 55)!")
                ...
                if challenge_mode in ["OPEN_CHALLENGE", "OBSTACLE_CHALLENGE"]:
                    ...
                    # ── LIDAR WATCHDOG SAFETY GUARD ──
                    # If LiDAR has stalled, disconnected, or dropped power for > 0.40s, immediately emergency stop!
                    time_since_lidar = time.time() - last_lidar_scan_time
                    if time_since_lidar > 0.40:
                        ser.write(b"S\nY 1\nG 0\nL 1\n")
                        print(f"[LIDAR TIMEOUT GUARD] No fresh scan for {time_since_lidar:.2f}s! Stopping bot to prevent wall collision.")
                        time.sleep(0.05)
                        continue
                ...
                if challenge_mode == "OPEN_CHALLENGE":
                    ...
                    if stuck_recovery_mode:
                        ...
                        # HALVED: 0.5s reverse (~18cm back at speed 55)
                        if dt_stuck < 0.5:
                            cmd = f"D -55 {stuck_reverse_dir}\n"
                            ser.write(cmd.encode('utf-8'))
                            action_name = "STUCK_REVERSE_BACKSTEP"
                            steer = stuck_reverse_dir
                            speed = -55
                        # HALVED: 0.7s realign forward (was 1.4s)
                        elif dt_stuck < 0.7:
                            fwd_steer = 50 if (stuck_reverse_dir > 110) else 170
                            cmd = f"D 45 {fwd_steer}\n"
                            ser.write(cmd.encode('utf-8'))
                            ...
                            stuck_recovery_mode = False
                            last_recovery_cooldown = time.time()
                    ...
                    # ── NORMAL OPEN CHALLENGE NAVIGATION ──
                    # Math: servo maps -30..+30 deg → PWM 170..50, center=110, scale=60/30=2 PWM/deg
                    # Left turn (negative offset → PWM<110), Right turn (positive → PWM>110)

                    # 1. Critical Side Wall Proximity Recovery (< 0.25m)
                    if 0.05 < left_dist < 0.25:
                        steer = 155
                        speed = max(40, base_spd - 8)
                        action_name = "EMERGENCY_RIGHT"
                    elif 0.05 < right_dist < 0.25:
                        steer = 65
                        speed = max(40, base_spd - 8)
                        action_name = "EMERGENCY_LEFT"
                    # 2. Corner Navigation (Front Wall within 0.75m)
                    #    Math: angle_to_wall = atan2(max_side_dist, front_dist)
                    #    Needed steer deg = clamp(angle_to_wall * (max_steer/90), 0, max_steer)
                    #    Translated to PWM: center ± (steer_deg * 2)
                    elif 0.05 < front_dist < 0.75:
                        urgency = max(0.0, min(1.0, (0.75 - front_dist) / 0.55))
                        speed = max(42, int(round(base_spd * (1.0 - 0.22 * urgency))))
                        max_side = max(left_dist, right_dist)
                        # angle needed to turn towards open side (degrees)
                        angle_to_open = math.degrees(math.atan2(max_side, max(front_dist, 0.05)))
                        # scale: 30 deg steer = full lock, proportional
                        steer_deg_needed = min(30.0, angle_to_open * 0.55 * (0.7 + 0.3 * urgency))
                        if left_dist >= right_dist:
                            steer = max(55, int(round(center_pwm - steer_deg_needed * 2.0)))
                            action_name = "CORNER_LEFT"
                        else:
                            steer = min(165, int(round(center_pwm + steer_deg_needed * 2.0)))
                            action_name = "CORNER_RIGHT"
                    # 3. Straight Track Centerline Tracking
                    else:
                        err = left_dist - right_dist
                        steer_adj = -int(round(30.0 * max(-1.0, min(1.0, err))))
                        gap_adj = -int(round(de_ang * 0.35))
                        total_adj = max(-45, min(45, steer_adj + gap_adj))
                        steer = max(65, min(155, center_pwm + total_adj))
                        speed = base_spd
                ...
                # STATE 2: OBSTACLE CHALLENGE (Evasion of Green & Red pillars + trajectory optimization)
                    ...
                    if lap_count >= 3:
                        challenge_mode = "IDLE"
                        ser.write(b"S\nY 1\nG 0\nL 0\nB\n")
                        print("[CHALLENGE SUCCESS] Completed 3 laps in Obstacle Challenge! Stopping bot.")
                        continue
                    ...
                    with state_lock:
                        ...
                        right_dist = latest_telemetry.get('lidar_right_m', 0.8)
                        front_dist = latest_telemetry.get('lidar_front_m', 1.5)
                        fl_dist = latest_telemetry.get('lidar_fl_m', 1.5)
                        fr_dist = latest_telemetry.get('lidar_fr_m', 1.5)

                    # ── COLOR-SPECIFIC PILLAR & WALL EVASIVE BACK-STEP RECOVERY ──
                    if stuck_recovery_mode:
                        dt_stuck = time.time() - stuck_start_time
                        # HALVED: 0.5s reverse (~18cm back at speed 55)
                        if dt_stuck < 0.5:
                            cmd = f"D -55 {stuck_reverse_dir}\n"
                            ser.write(cmd.encode('utf-8'))
                            action_name = f"BACKSTEP_REVERSE_{stuck_reason}"
                            steer = stuck_reverse_dir
                            speed = -55
                        # HALVED: 0.7s realign forward (was 2.0s)
                        elif dt_stuck < 0.7:
                            fwd_steer = 55 if (stuck_reverse_dir > 110) else 165
                            cmd = f"D 45 {fwd_steer}\n"
                            ser.write(cmd.encode('utf-8'))
                            action_name = f"LANE_SHIFT_{stuck_reason}"
                            steer = fwd_steer
                            speed = 45
                        else:
                            stuck_recovery_mode = False
                            last_recovery_cooldown = time.time()
                            print(f"[RECOVERY COMPLETE] Created lateral clearance for {stuck_reason}! Resuming navigation.")
                    ...
                    # Cooldown check: prevent re-triggering back-step within 1.2s of completing recovery
                    now_t = time.time()
                    in_cooldown = (now_t - last_recovery_cooldown < 1.2)

                    # ── Proximity & Collision Risk Detection for Pillars & Walls ──
                    detected_hazard = False
                    hazard_steer_reverse = 110
                    hazard_type = ""
                    ...
                    if detected_hazard:
                        stuck_recovery_mode = True
                        stuck_start_time = time.time()
                        stuck_reverse_dir = hazard_steer_reverse
                        stuck_reason = hazard_type
                        cmd = f"D -55 {stuck_reverse_dir}\n"
                        ser.write(cmd.encode('utf-8'))
                        continue

                    # Compute optimal trajectory avoiding red/green obstacles with rule adherence & LiDAR point cloud safety
                    plan_res = fast_planner.plan_optimal_trajectory(
                        lidar_points=pts,
                        towers=towers,
...
def main():
    ...
    # Stagger USB device startup to prevent bus brownout from 3 devices drawing current at once
    print("[STARTUP] Starting LiDAR thread (USB power stabilisation stagger)...")
    t_lidar.start()
    time.sleep(1.5)   # Let LiDAR motor spin up fully before ESP32 wakes
    print("[STARTUP] Starting ESP32 thread...")
    t_esp.start()
    time.sleep(1.0)   # Let ESP32 serial link stabilise before camera opens
    print("[STARTUP] Starting Camera thread...")
    t_cam.start()

    with config_lock:
        web_enabled = ("--web" in sys.argv) or config_data.get('general', {}).get('web_dashboard_enabled', False)

    print("=" * 60)
    if web_enabled:
        print("  WRO Autonomous Navigation Engine Running (Web Dashboard ACTIVE)")
        print("  Open in browser: http://192.168.137.111:5000")
    else:
        print("  WRO Autonomous Navigation Engine Running (Headless Competition Mode)")
        print("  Web server is OFF by default for maximum performance & power stability.")
        print("  (To enable web server, run with --web flag)")
    print("  - Button 1: Start Obstacle Challenge (3 Laps)")
