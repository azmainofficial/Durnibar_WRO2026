 ...
 * @brief Arduino IDE Sketch for Team Durnibar WRO 2026 ESP32 Vehicle Controller
 * @team Team Durnibar (Bangladesh)
 * 
 * Hardware Pin Mapping:
 * - TB6612FNG Motor Driver: PWMA=25, AIN1=26, AIN2=27, STBY=23
 * - Quadrature Encoder:      Encoder A=34 (Interrupt), Encoder B=35
 * - Steering Servo:          Servo=18 (PWM 50Hz)
 * - Status Indicators:       Buzzer=19, LED Green=16, LED Yellow=17, LED Red=5
 * - Push Buttons:            Button 1 (Start)=13, Button 2=12, Button 3=14 (INPUT_PULLUP)
...
#define PIN_BUTTON_1    13 // WRO 2026 Rule 9.11 Main Start Button
...
// Steering Angle Safety Limits (Ackermann Protection)
...
void setup() {
    ...
    beepBuzzer(1000, 150);
    Serial.println("[ESP32] Hardware Setup Complete. Waiting for Button 1 (WRO Rule 9.11)...");
}
...
void loop() {
    ...
    // 2. Parse Control Commands from Raspberry Pi 5 Serial
    ...
    // 3. Telemetry Stream at 50 Hz to Raspberry Pi 5
    ...
    if (now - lastTelemetryTime >= 20) {
        ...
        // Telemetry Packet: "$TEL,<Ticks>,<GyroZ>,<AccelX>,<ActiveState>#"
