...
class FastAckermannTrajectoryOptimizer:
    def __init__(self,
        ...
        self.weights = {
            "w_clearance": 6.0,       # Strict repulsion from walls and obstacles
            ...
            "safety_margin_m": 0.22,  # Desired clearance buffer around robot
    ...
    def plan_optimal_trajectory(self,
        ...
        # Hard proximity repulsion if getting too close to either side wall
        if 0.05 < left_wall_m < 0.32:
            # Dangerously close to LEFT wall -> penalize left steer (>0) and strongly favor right steer (<0)
            left_wall_danger = (0.32 - left_wall_m) / 0.32
            steer_bias = np.where(self.steer_angles_deg > 0, self.steer_angles_deg * 2.5, -self.steer_angles_deg * 1.5)
            costs[valid_mask] += 120.0 * left_wall_danger * (steer_bias[valid_mask] / self.max_steer_deg)

        if 0.05 < right_wall_m < 0.32:
            # Dangerously close to RIGHT wall -> penalize right steer (<0) and strongly favor left steer (>0)
            right_wall_danger = (0.32 - right_wall_m) / 0.32
            steer_bias = np.where(self.steer_angles_deg < 0, -self.steer_angles_deg * 2.5, self.steer_angles_deg * 1.5)
            costs[valid_mask] += 120.0 * right_wall_danger * (steer_bias[valid_mask] / self.max_steer_deg)
        ...
        # In this chassis: Left turn increases angle towards 170, Right turn decreases towards 50
