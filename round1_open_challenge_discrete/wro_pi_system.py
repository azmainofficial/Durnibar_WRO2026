    ...
    while True:
        try:
            ...
            while True:
                ...
                # STATE 1: OPEN CHALLENGE (Discrete Cornering State Machine)
                if challenge_mode == "OPEN_CHALLENGE":
                    ...
                    if stuck_recovery_mode:
                        ...
                        if dt_stuck < 0.9:
                            ...
                            stuck_recovery_mode = False
                            discrete_state = "DRIVE_STRAIGHT"  # Force back to straight drive
                            print("[RECOVERY COMPLETE] Back step done! Resuming Straight Navigation.")
                    ...
                    if discrete_state == "DRIVE_STRAIGHT":
                        ...
                        if 0.05 < front_dist < front_turn_threshold:
                            # Decide turn direction based on which side has more room
                            if left_dist >= right_dist:
                                turn_direction = 1  # Left
                                steer = 60          # Full Left
                                action_name = "DISCRETE_TURN_LEFT"
                                print(f"[NAVIGATION] Corner ahead! front={front_dist:.2f}m. More space on Left. Initiating 90° Turn Left...")
                            else:
                                turn_direction = -1 # Right
                                steer = 160         # Full Right
                                action_name = "DISCRETE_TURN_RIGHT"
                                print(f"[NAVIGATION] Corner ahead! front={front_dist:.2f}m. More space on Right. Initiating 90° Turn Right...")
                                
                            turn_start_yaw = esp_yaw
                            discrete_state = "TURNING_90"
                            speed = corner_speed
                        else:
                            # Drive straight with gentle side corrections to stay centered
                    ...
                    # ── STATE: TURNING_90 ──
                        ...
                        if yaw_change_mag >= target_turn_angle:
                            ...
                            turn_direction = 0
                            print(f"[NAVIGATION] 90° Turn completed! (Yaw change = {yaw_change_mag:.1f}°). Resuming straight drive.")
