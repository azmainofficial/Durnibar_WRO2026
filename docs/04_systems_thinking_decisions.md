...
| **Vision Processor** | Edge Cloud Vision (Offboard) | **Fully Onboard Processing (SBC)** | Offboard processing reduces vehicle weight, but introduces wireless latency ($>120\text{ ms}$) and violates Rule 11.10 (no wireless communication allowed during rounds). Fully onboard SBC keeps latency $<33\text{ ms}$ ($30\text{ FPS}$). |
